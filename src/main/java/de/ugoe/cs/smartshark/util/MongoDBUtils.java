	...
	private static ArrayList<StructField> parseSchema(BasicDBObject[] fields, List<String> types) {
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if ((types == null) || (types.contains(logical_type))) {
				...
				{
					structFields.add(
							DataTypes.createStructField((String) field.get("field_name"), DataTypes.StringType, true));
					break;
				}
				case "IntegerType":
				{
					structFields.add(
							DataTypes.createStructField((String) field.get("field_name"), DataTypes.IntegerType, true));
					break;
				}
				case "BooleanType":
				{
					structFields.add(
							DataTypes.createStructField((String) field.get("field_name"), DataTypes.BooleanType, true));
					break;
				}
				case "DateType":
				{
					structFields.add(
							DataTypes.createStructField((String) field.get("field_name"), DataTypes.DateType, true));
					break;
				}
				case "StructType":
				{
					...
					structFields.add(DataTypes.createStructField((String) field.get("field_name"),
				...
				case "ArrayType":
				{
					...
					{
						structFields.add(DataTypes.createStructField((String) field.get("field_name"),
								DataTypes.createArrayType(DataTypes.BooleanType, true), true));
						break;
					}
					case "DateType":
					{
						structFields.add(DataTypes.createStructField((String) field.get("field_name"),
								DataTypes.createArrayType(DataTypes.DateType, true), true));
						break;
					}
